...
def main_loop(n, m_frac, Cooling_water, Flue_gas, CW_flowrate, steam_flowrate, m_g, a):
    ...
    for i in range(n):
        ...
        if T_w < T_sat and i < len(Temperature_interface):
            T_gout = ((m_g * c_pg * 1000 - (h_g/2) * delta_Ai) * T_gin + h_g * delta_Ai * Temperature_interface[i]) / \
...
results = main_loop(n,steam_flowrate/(steam_flowrate+Air_flowrate),Cooling_water,Flue_gas,CW_flowrate,steam_flowrate,steam_flowrate+Air_flowrate,a)
